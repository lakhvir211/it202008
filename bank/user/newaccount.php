...
if (!$_SESSION) {
 ...
 echo ('<h2>Please log in First</h2>');
  ...
  $accountNumber= rand(111111111111, 999999999999);
...
if (empty($_POST["acount-name"])) {
    ...
    $acountName = test_input($_POST["acount-name"]);
        ...
        $stmt = $pdo->prepare("SELECT acount_name FROM account WHERE acount_name=?");
        $stmt->execute([$acountName]);
        $user = $stmt->fetch();
        if($user){
            $nameError = "Account already in use";
            $acountName="";
        }
        else{
            $nameError="";
  ...
  <div class="content">
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
               <div class="form-group">
                  ...
                  <input type="text" class="form-control" id="acount-name" placeholder="Account User Name" name="acount-name" value="<?php echo $acountName?>">
